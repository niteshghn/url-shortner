...
def redirect_to_source(request, hash):
    url = get_object_or_404(UrlKeyHash, hash=hash)
    url.hits = url.hits + 1
    url.save()
...
def get_short_hash():
    ...
    while True:
        ...
        try:
            UrlKeyHash.objects.get(hash=code)
...
def make_tiny_url(request):
    ...
    UrlKeyHash.objects.create(url=long_url, hash=short_hash, user=request.user, is_private=is_private)
